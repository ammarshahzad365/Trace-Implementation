"""The HTTP API: three endpoints over the same engine the batch loader uses.
    ...
    GET  /schema     which entity types and relationship types are accepted
...
It does not preprocess. Whatever fields a record carries become properties under
those names, exactly as in `graphload/properties.py`. It is the same contract as
the files under `data-preprocessing/`, which means anything valid there is valid
here, and vice versa.
"""
...
# IngestError for a missing or undeclared field, PropertyError for a value Neo4j
...
@app.get("/schema")
def schema() -> dict:
    """The accepted entity types, and the relationship types already in use."""
    with _session() as handle:
        in_use = [
            row["t"]
    ...
    return {
        "entity_types": dict(sorted(catalog.LABELS.items())),
        "relationship_types_in_use": sorted(in_use),
        "allow_new_labels": STATE.get("allow_new_labels", False),
        "note": (
            "An entity 'type' must be a key of entity_types unless the server was started "
            "with --allow-new-labels. A relationship_type is free-form and is uppercased."
        ),
    }
