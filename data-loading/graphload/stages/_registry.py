...
that weren't loaded in this run get filled in from their pickle cache -- or
scanned if the cache is missing or stale.

This is why the cache is per source rather than one file: `--stage bridges`
resolves 1.1M ids in a couple of seconds instead of re-streaming ~700 MB of
CVE JSON.
"""
...
def ensure(ctx: Context) -> dict:
    ...
    for spec in ctx.all_specs:
        if spec.key in ctx.covered_sources:
            continue
        entries = load_cached(ctx.settings.cache_dir, spec, ctx.repo_root) if ctx.use_cache else None
        if entries is None:
    ...
    ctx.log(f"  registry holds {len(ctx.registry):,} ids across {len(ctx.registry.labels())} label(s)")
