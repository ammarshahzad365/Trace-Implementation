...
Which fields carry those things is not assumed -- it's declared, via
`RecordShape`/`EdgeShape`. The defaults happen to match what
`data-preprocessing/` emits (`id`, `type`, `relationship_type`, `source_ref`,
`target_ref`), so `catalog/` never has to spell them out; a future dataset that
calls them `uuid`/`kind`/`from`/`to` passes a different shape rather than
needing a different loader.
