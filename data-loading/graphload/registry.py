...
thing that is. Two jobs need to know:

- **The edge stages** must pick a label to `MATCH` on, because a lookup scoped
  to `:Vulnerability` uses that label's index and an unscoped one scans the
  whole store.
- **`router.py`** must decide whether an edge stays inside one source or crosses
  between two -- the difference between the `edges` stage and the `bridges`
  stage, and something that cannot be read off a filename. Only 1,310 of
  D3FEND's 5,056 edges are D3FEND-to-D3FEND; the rest point into ATT&CK and CWE.

Memory matters here, because this map holds every id in the graph. So labels and
sources are interned into a small table and each id maps to a single small
integer, rather than to a tuple of two strings.

## The cache

Rebuilding the map means streaming every entity file, and `CVE/entities.json`
alone is 402 MB of that. Since the whole point of the design is that adding a
sixth source should not mean re-reading the first five, the cache is **per
source**, keyed by a fingerprint of that source's entity files (path, size,
mtime). Touch one file in one source and only that source is rescanned.
"""
...
class Registry:
    ...
    def add(self, entity_id: str, label: str, source: str) -> None:
        ...
        if existing is not None:
            # Any repeat is a problem, whether or not it is the same label: two
...
def save_cached(
    cache_dir: Path, spec: SourceSpec, repo_root: Path, entries: Sequence[tuple[str, str]]
) -> None:
        ...
        pickle.dump(
            {
                ...
                "fingerprint": fingerprint(spec, repo_root),
