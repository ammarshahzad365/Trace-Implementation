...
id, so loading 1.13M relationships means 2.27M lookups of the form
"find the node with this id". Backed by the index a constraint creates, each is
an instant seek. Unbacked, each is a scan of a million-node label -- the
difference between a load measured in minutes and one measured in hours.

A constraint also does the second job it's actually named for: it makes it
impossible for two records to quietly fuse into one node by sharing an id.
`validate.py` checks for that in Python too, so the failure is reported with
both offending records rather than as a driver exception, but the constraint is
the backstop.
...
def existing_constraints(handle: Session) -> set[str]:
    return {row["name"] for row in handle.run("SHOW CONSTRAINTS YIELD name RETURN name")}


def drop_constraint(handle: Session, name: str) -> None:
    handle.run(f"DROP CONSTRAINT {name} IF EXISTS").consume()
