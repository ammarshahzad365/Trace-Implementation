...
Six stages, each independently runnable. Order is a dependency chain, not a
...
from graphload.dedupe import Canonicalizer
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--stage", nargs="+", choices=stages.ORDER, metavar="STAGE",
                        help=f"run only these stages (default: all, in order: {' '.join(stages.ORDER)})")
    parser.add_argument("--only", nargs="+", choices=keys, metavar="SOURCE",
                        help="load only these sources (the bridges stage always reads all of them)")
    parser.add_argument("--skip", nargs="+", choices=keys, metavar="SOURCE", default=(),
                        help="skip these sources")
    parser.add_argument("--dry-run", action="store_true",
                        help="validate, route and report without writing anything")
    ...
    parser.add_argument("--allow-new-labels", action="store_true",
                        help="accept a mechanically derived label for a type that isn't in "
                             "catalog/labels.py, instead of stopping")
    parser.add_argument("--no-cache", action="store_true",
                        help="ignore the per-source registry cache and rescan")
    parser.add_argument("--check", action="store_true",
                        help="report connectivity and what's already in the database, then exit")
...
def selected_sources(args: argparse.Namespace):
    chosen = catalog.SOURCES if args.only is None else tuple(
        s for s in catalog.SOURCES if s.key in args.only
    )
    return [s for s in chosen if s.key not in args.skip]


def self_check() -> int:
    """The engine must not depend on the catalog. That's what makes it reusable."""
    ...
    print(f"OK -- catalog declares {len(catalog.SOURCES)} sources, "
          f"{len(catalog.all_labels())} labels, {len(catalog.RULES)} cross-catalog rules, "
          f"{len(catalog.ENRICHMENTS)} enrichment steps")
    return 0
...
def build_context(args: argparse.Namespace, log) -> Context:
    ...
    return Context(
        ...
        canonicalizer=Canonicalizer(rules=catalog.RULES),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    to_run = list(args.stage) if args.stage else list(stages.ORDER)
    to_run.sort(key=stages.ORDER.index)

    print(f"Target: {ctx.settings.redacted}")
    print(f"Sources: {', '.join(s.label for s in ctx.selected_specs)}")
    print(f"Stages: {' -> '.join(to_run)}" + ("   [DRY RUN -- nothing will be written]" if ctx.dry_run else ""))
    if ctx.limit:
    ...
    try:
        ...
        for name in to_run:
            ...
            try:
                module = stages.MODULES[name]
                if name == "enrich":
                    results[name] = module.run(ctx, handle, catalog.ENRICHMENTS)
                else:
                    results[name] = module.run(ctx, handle)
            except SystemExit:
                raise
            ...
            # Gate right after the first stage that reads records, so a bad
            # dataset stops before edges are written rather than after.
            if name == "nodes":
                raise_if_fatal(ctx.findings, ctx.allow_new_labels)
                collisions = ctx.collisions.problems()
                if collisions:
                    ctx.findings.property_collisions = collisions
                    raise_if_fatal(ctx.findings, ctx.allow_new_labels)
            if name == "bridges":
                raise_if_fatal(ctx.findings, ctx.allow_new_labels)
    finally:
