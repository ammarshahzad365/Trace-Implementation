...
Downloads MITRE's current CAPEC STIX 2.1 bundle, compares it against the local
snapshot, and rewrites `latest.json` as the new full snapshot. Reports objects
added, modified, or removed relative to what was stored locally.
"""
...
def sync(
    ...
    print("[capec] diffing against local snapshot...")
    ...
    if not dry_run:
        print("[capec] writing latest.json...")
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch the complete CAPEC dataset as STIX 2.1")
    parser.add_argument(
        "--base-dir",
