"""Incremental CAPEC crawler.

CAPEC has no date-filtered fetch endpoint -- MITRE only publishes the full current
bundle. This crawler downloads that same full bundle (same network cost as the full
crawler), but instead of overwriting the local snapshot, it merges the fetch into
the existing `latest.json` and writes only the objects added or modified since the
last successful fetch to `delta.json`. Requires a prior run of `full_crawler.py`
(there must be a `last_successful_fetch` in manifest.json to be "incremental" from).
"""
...
def sync(
    ...
    bundle = client.fetch_bundle(source_url, timeout=timeout, user_agent=user_agent)
    objects = [obj for obj in bundle["objects"] if isinstance(obj, dict)]
    print(f"[capec] parsed {len(objects)} objects: {client.object_type_counts(objects)}")

    incoming_state = client.normalize_objects(objects)
    print("[capec] merging into local snapshot...")
    ...
    result: Dict[str, Any] = {
        ...
        "capec_version": client.capec_version(objects),
    ...
    if not dry_run:
        print("[capec] writing latest.json and delta.json...")
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch CAPEC and merge only what changed since the last successful fetch")
    parser.add_argument(
        "--base-dir",
    ...
    parser.add_argument(
        ...
        help=f"CAPEC STIX bundle URL (default: {client.DEFAULT_SOURCE_URL})",
    )
    parser.add_argument(
        "--timeout",
        type=int,
