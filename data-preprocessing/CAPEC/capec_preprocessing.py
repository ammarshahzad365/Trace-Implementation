...
def read_collected_at(manifest_path: Path) -> str | None:
    ...
    try:
        ...
        print(f"[capec-parser] warning: no readable manifest at {manifest_path}; records get no collected_at", file=sys.stderr)
        return None
    collected_at = manifest.get("last_successful_fetch") or manifest.get("generated_at")
    if not collected_at:
        print(f"[capec-parser] warning: {manifest_path.name} has no fetch timestamp; records get no collected_at", file=sys.stderr)
    return collected_at
