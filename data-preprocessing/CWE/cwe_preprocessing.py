...
# ObservedExamples get their own type rather than the generic `related_to` above. NVD's
# own `CVE-N -> CWE-N` assignment (the CVE module's 336,339 `related_to` links) is a
# different claim -- "this vulnerability is an instance of this weakness" -- from CWE
# citing a CVE to illustrate itself. Sharing one type collapsed 932 of these onto an
# NVD link of the opposite direction, which in a MERGE-based load would have dropped
# the `description`/`link` payload that only this side carries.
OBSERVED_EXAMPLE_RELATIONSHIP_TYPE = "has_observed_example"
...
def read_collected_at(manifest_path: Path) -> str | None:
    ...
    try:
        ...
        print(f"[cwe-parser] warning: no readable manifest at {manifest_path}; records get no collected_at", file=sys.stderr)
        return None
    collected_at = manifest.get("last_successful_fetch") or manifest.get("generated_at")
    if not collected_at:
        print(f"[cwe-parser] warning: {manifest_path.name} has no fetch timestamp; records get no collected_at", file=sys.stderr)
    return collected_at
...
def build_observed_example_relationships(obj: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    dropped, since outward-pointing edges are scoped to CVE/CAPEC. Typed
    `has_observed_example`, not `related_to` -- see the constant for why."""
    source_ref = f"CWE-{obj['cwe_id']}"
    relationships = []
    for item in as_list(obj.get("ObservedExamples", {}).get("ObservedExample")):
        ...
        relationships.append(make_relationship(source_ref, target_ref, OBSERVED_EXAMPLE_RELATIONSHIP_TYPE, **extra))
    return relationships
...
def collapse_parallel_relationships(records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    attributes -- the same `ChildOf` parent recorded under four different `view_id`s. Written straight through, those became parallel
